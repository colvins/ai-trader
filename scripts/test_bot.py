...
def build_ai_focus_prompt_message(scan_results) -> str:
    ...
    lines = [
        ...
        "",
        "请作为日本短线交易员，而不是普通分析师。",
        "我已经先用本地系统筛出了 3~5 只候选股，你不需要重新全市场选股，只需要基于这些股票做“短线交易优先级判断”。",
        "",
        "你的任务：",
        "",
        "1. 先对以下股票按“明日短线交易优先级”从高到低排序",
        "2. 再逐只给出结论",
        "",
        "请严格按以下要求回答：",
        "",
        ...
        "对每只股票只回答以下 5 项：",
        "",
        "1. 交易结论：只允许写「可买 / 观察 / 放弃」",
        "2. 更偏模式：只允许写「breakout / trend / dip / 不明确」",
        "3. 理想入场方式：只允许写「平开跟随 / 回踩低吸 / 突破确认 / 不建议参与」",
        "4. 核心逻辑：一句话",
        "5. 最大风险：一句话",
        "",
        "额外要求：",
        ...
        "* 不要重复解释基本概念",
        "* 不要给中长线分析",
        "* 重点考虑短线交易、量价配合、动量延续性、追高风险、次日可执行性",
        "* 如果某只股票虽然强，但不适合明日短线参与，也要明确写「观察」或「放弃」",
        "",
        "以下是候选股票：",
        "",
    ]
